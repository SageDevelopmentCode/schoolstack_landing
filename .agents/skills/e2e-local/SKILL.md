...
5. If port 3000 has a dev server that may use prod `.env.local`, stop it before `npm run test:e2e`
...
npm run test:e2e         # 7 tests; starts dev:next automatically
npm run test:e2e:ui      # interactive mode
```

**Expected:** 7 passed (1 setup + 3 smoke + 3 authenticated).

## What runs

1. **globalSetup** (`e2e/global-setup.ts`) — env guards + seed users/data via `e2e/fixtures/seed.ts`
