-- Promoted to supabase/migrations/20260809_add_portal_message_attachments.sql for local/CI.
...
drop policy if exists "Platform admins manage portal_message_attachments" on public.portal_message_attachments;
...
drop policy if exists "Org admins manage portal_message_attachments" on public.portal_message_attachments;
create policy "Org admins manage portal_message_attachments"
  on public.portal_message_attachments for all to authenticated
  using (public.user_is_org_admin(organization_id))
  with check (public.user_is_org_admin(organization_id));

drop policy if exists "Participants read portal_message_attachments" on public.portal_message_attachments;
...
drop policy if exists "Participants insert portal_message_attachments" on public.portal_message_attachments;
...
drop policy if exists "Platform admins manage portal-message-files storage" on storage.objects;
...
drop policy if exists "Org admins manage portal-message-files storage" on storage.objects;
...
drop policy if exists "Participants read portal-message-files storage" on storage.objects;
...
drop policy if exists "Participants insert portal-message-files storage" on storage.objects;
