-- Promoted to supabase/migrations/20260810_add_web_push_subscriptions.sql for local/CI.
...
drop policy if exists "Users manage own web_push_subscriptions" on public.web_push_subscriptions;
create policy "Users manage own web_push_subscriptions"
  on public.web_push_subscriptions for all to authenticated
  using (user_id = auth.uid())
  with check (user_id = auth.uid());

drop policy if exists "Platform admins manage web_push_subscriptions" on public.web_push_subscriptions;
