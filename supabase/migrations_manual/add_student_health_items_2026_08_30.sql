-- Promoted to supabase/migrations/20260837_add_student_health_items.sql for local/CI.
-- Run this file in Supabase SQL Editor on remote if that migration has not been applied.
-- Purpose: parent portal student health items (allergies, medications, updates)
...
drop policy if exists "Platform admins manage student_health_items" on public.student_health_items;
...
drop policy if exists "Org admins manage student_health_items" on public.student_health_items;
create policy "Org admins manage student_health_items"
  on public.student_health_items for all to authenticated
  using (public.user_is_org_admin(organization_id))
  with check (public.user_is_org_admin(organization_id));

drop policy if exists "Guardians read student_health_items for own students" on public.student_health_items;
