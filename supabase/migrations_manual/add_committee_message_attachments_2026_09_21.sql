-- Promoted to supabase/migrations/20261018_add_committee_message_attachments.sql for local/CI.
...
drop policy if exists "Platform admins manage committee_message_attachments" on public.committee_message_attachments;
...
drop policy if exists "Org admins manage committee_message_attachments" on public.committee_message_attachments;
create policy "Org admins manage committee_message_attachments"
  on public.committee_message_attachments for all to authenticated
  using (public.user_is_org_admin(organization_id))
  with check (public.user_is_org_admin(organization_id));

drop policy if exists "Committee members read committee_message_attachments" on public.committee_message_attachments;
...
drop policy if exists "Committee members insert committee_message_attachments" on public.committee_message_attachments;
...
drop policy if exists "Platform admins manage committee-message-files storage" on storage.objects;
...
drop policy if exists "Org admins manage committee-message-files storage" on storage.objects;
...
drop policy if exists "Staff read committee-message-files storage" on storage.objects;
...
drop policy if exists "Committee members read committee-message-files storage" on storage.objects;
...
drop policy if exists "Committee members upload committee-message-files storage" on storage.objects;
