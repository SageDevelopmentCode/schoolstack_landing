-- Promoted to supabase/migrations/20260854_add_program_coop_curriculum.sql for local/CI.
...
drop policy if exists "Platform admins manage program_coop_curriculum" on public.program_coop_curriculum;
...
drop policy if exists "Org admins manage program_coop_curriculum" on public.program_coop_curriculum;
create policy "Org admins manage program_coop_curriculum"
  on public.program_coop_curriculum for all to authenticated
  using (public.user_is_org_admin(organization_id))
  with check (public.user_is_org_admin(organization_id));

drop policy if exists "Staff read program_coop_curriculum" on public.program_coop_curriculum;
...
drop policy if exists "Enrolled guardians read program_coop_curriculum" on public.program_coop_curriculum;
...
drop policy if exists "Platform admins manage program-coop-curriculum-files storage" on storage.objects;
...
drop policy if exists "Org admins manage program-coop-curriculum-files storage" on storage.objects;
...
drop policy if exists "Staff read program-coop-curriculum-files storage" on storage.objects;
...
drop policy if exists "Enrolled guardians read program-coop-curriculum-files storage" on storage.objects;
