-- Promoted to supabase/migrations/20260928_add_teacher_parent_forms.sql for local/CI.
...
drop policy if exists "Platform admins manage teacher_parent_forms" on public.teacher_parent_forms;
...
drop policy if exists "Org admins manage teacher_parent_forms" on public.teacher_parent_forms;
create policy "Org admins manage teacher_parent_forms"
  on public.teacher_parent_forms for all to authenticated
  using (public.user_is_org_admin(organization_id))
  with check (public.user_is_org_admin(organization_id));

drop policy if exists "Org members read teacher_parent_forms" on public.teacher_parent_forms;
...
drop policy if exists "Platform admins manage teacher_parent_form_responses" on public.teacher_parent_form_responses;
...
drop policy if exists "Org admins manage teacher_parent_form_responses" on public.teacher_parent_form_responses;
create policy "Org admins manage teacher_parent_form_responses"
  on public.teacher_parent_form_responses for all to authenticated
  using (public.user_is_org_admin(organization_id))
  with check (public.user_is_org_admin(organization_id));

drop policy if exists "Org members read teacher_parent_form_responses" on public.teacher_parent_form_responses;
...
drop policy if exists "Guardians read teacher_parent_form_responses for own family" on public.teacher_parent_form_responses;
...
drop policy if exists "Platform admins manage teacher-parent-form-files storage" on storage.objects;
...
drop policy if exists "Org admins manage teacher-parent-form-files storage" on storage.objects;
...
drop policy if exists "Staff read teacher-parent-form-files storage" on storage.objects;
