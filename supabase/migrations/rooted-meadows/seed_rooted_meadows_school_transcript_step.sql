-- Add School Transcript file-upload step to Rooted Meadows enrollment checklist.
...
declare
  ...
  v_item_key text := 'school_transcript';
...
begin
  ...
  if exists (
    ...
    raise notice 'School Transcript step already exists — skipping.';
    return;
  end if;

  -- Place immediately after Immunization Records.
  select coalesce(
    (
  ...
  raise notice 'Added School Transcript step at sort_order %.', v_sort_order;
end $$;
...
select
  ...
  i.type,
  i.item_key,
  jsonb_array_length(i.metadata->'fileUpload'->'directions'->'options') as direction_count,
