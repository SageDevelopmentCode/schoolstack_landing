...
- **Android:** `actions/cache` keyed on `package-lock.json`, `apps/mobile/package.json`, `apps/mobile/app.json`, `apps/mobile/src/**`, `apps/mobile/.maestro/**`, and `apps/mobile/scripts/build-android-debug-ci.sh` (with `restore-keys: mobile-android-e2e-` for partial hits) — caches Gradle deps and the generated `android/` tree. A debug APK is reused only when `app-debug.apk.e2e-stamp` matches the current cache key hash (same inputs as the cache key); otherwise CI deletes the stale APK and rebuilds with an **embedded E2E JS bundle** (`debuggableVariants = []` patch + `assembleDebug` in [`build-android-debug-ci.sh`](../../apps/mobile/scripts/build-android-debug-ci.sh)) so Maestro does not depend on Metro on the emulator. [`run-e2e-android-ci.sh`](../../apps/mobile/scripts/run-e2e-android-ci.sh) installs the APK and runs Maestro with `adb reverse` for Next.js (`3100`) and Supabase (`54321`) only.
