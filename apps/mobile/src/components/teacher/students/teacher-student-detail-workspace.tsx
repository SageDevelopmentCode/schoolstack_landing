...
import { tabPanelEntering, tabPanelExiting } from '@/lib/motion/portal-motion';
import { getSupabaseClient } from '@/lib/supabase';
import { useMobileErrorReporter } from '@/lib/use-mobile-error-reporter';

type TeacherStudentDetailWorkspaceProps = {
  studentId: string;
...
export function TeacherStudentDetailWorkspace({
  studentId,
  detailAccess = 'assigned',
  variant,
  onClose,
}: TeacherStudentDetailWorkspaceProps) {
  const theme = useParentTheme();
  ...
  const loadDetail = useCallback(async () => {
    if (!organizationId) {
      setError('School profile not loaded.');
      setLoading(false);
      return;
    }

    if (detailAccess === 'assigned' && !staffMemberId) {
    ...
    try {
      const nextDetail =
        detailAccess === 'school'
          ? await loadTeacherSchoolStudentDetail(supabase, organizationId, studentId)
          : await loadTeacherAssignedStudentDetail(
              supabase,
              organizationId,
              staffMemberId,
              studentId,
  ...
  }, [detailAccess, organizationId, reportError, staffMemberId, studentId, supabase]);
      ...
      <StoryDetailSection
        ...
        <View style={styles.fieldGrid}>
          ...
          {detailAccess === 'school' ? (
            <DetailField
              label="Teachers"
              value={panelDetail.assignedTeacherNames.trim() || 'Unassigned'}
            />
          ) : null}
          <DetailField
            label="Date of birth"
  ...
  const sheetTabPanel =
    visitedTabs.has(activeTab) ? (
      <Animated.View
        key={activeTab}
        entering={tabPanelEntering}
        exiting={tabPanelExiting}>
  ...
  return (
    <SafeAreaView style={[styles.container, { backgroundColor: Story.paper }]} edges={['top']}>
      {screenHeader}
      <SubmissionStoryTabBar tabs={tabs} activeTabId={activeTab} onChange={navigateToTab} />
      <ScrollView contentContainerStyle={styles.scrollContent}>{screenTabPanels}</ScrollView>
    </SafeAreaView>
  );
}
