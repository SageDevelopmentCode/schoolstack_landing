    ...
    <View style={styles.container}>
      ...
      <StoryCard compact style={styles.panelCard}>
        <View style={styles.panelHeader}>
          ...
          <StoryTextLink label="View all →" onPress={() => onNavigate('calendar')} />
        </View>
        {upcomingEvents.length === 0 ? (
          <Text style={[styles.emptyCopy, { color: theme.muted }]}>No upcoming dates.</Text>
        ) : (
          upcomingEvents.map((event) => (
            <View key={event.id} style={[styles.eventRow, { borderColor: '#E0E7E0', backgroundColor: theme.white }]}>
      ...
      <StoryCard compact style={styles.panelCard}>
        <View style={styles.panelHeader}>
          ...
          <StoryTextLink label="View tasks →" onPress={() => onNavigate('tasks')} />
        </View>
        {urgentTasks.length === 0 ? (
          <Text style={[styles.emptyCopy, { color: theme.muted }]}>No open tasks.</Text>
        ) : (
          urgentTasks.map((task) => (
            <View key={task.id} style={[styles.taskRow, { borderColor: '#E0E7E0', backgroundColor: theme.white }]}>
