...
import { staffDisplayName } from '@/lib/school-admin/staff-labels';
...
function summarizeStaffPortalLoginStatus(members: StaffMemberRecord[]) {
  let signedIn = 0;

  for (const member of members) {
    if (!staffHasPortalAccess(member)) continue;
    if (member.isLinked && member.hasEverSignedIn) {
      signedIn += 1;
    }
  }
...
export function deriveStaffRosterMetrics(members: StaffMemberRecord[]): StaffRosterMetrics {
