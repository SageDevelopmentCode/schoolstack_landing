...
export function isStudentUnassigned(
  student: Pick<AdminEnrolledStudentSummary, 'assignedTeachers' | 'classroomNames'>,
): boolean {
  return student.classroomNames.length === 0 || student.assignedTeachers.length === 0;
}

export function isRecentEnrollment(enrolledAt: string, nowMs: number = Date.now()): boolean {
