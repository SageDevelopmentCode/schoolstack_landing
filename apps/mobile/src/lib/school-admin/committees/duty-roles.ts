...
import { mapDutyRoleRow } from '@/lib/school-admin/committees/mappers';
...
export async function deleteDutyRole(
  ...
  const { error } = await supabase.from('committee_duty_roles').delete().eq('id', dutyRoleId);
  if (error) throw new Error(error.message);
  ...
  for (const resource of resources ?? []) {
    ...
    const nextIds = currentIds.filter((id) => id !== dutyRoleId);
