...
export const DEFAULT_PORTAL_CACHE_TTL_MS = 2 * 60 * 1000;
...
export function registerPortalMemoryClear(handler: () => void): void {
  memoryClearHandlers.add(handler);
}

export function unregisterPortalMemoryClear(handler: () => void): void {
  memoryClearHandlers.delete(handler);
}

export async function clearAllPersistedPortalCaches(): Promise<void> {
...
export function createPortalCache<T>(diskCachePrefix: string): PortalCache<T> {
  ...
  registerPortalMemoryClear(clearMemory);
  ...
  return {
    ...
    isStale(key: string, ttlMs = DEFAULT_PORTAL_CACHE_TTL_MS): boolean {
...
export type PortalProviderInitResult<T> = {
  data: T | null;
  hasLoaded: boolean;
  isLoading: boolean;
...
export async function resolvePortalProviderInit<T>(
  key: string,
  cache: PortalCache<T>,
  hydrateFromDisk: () => Promise<T | null>,
  ttlMs = DEFAULT_PORTAL_CACHE_TTL_MS,
): Promise<PortalProviderInitResult<T>> {
