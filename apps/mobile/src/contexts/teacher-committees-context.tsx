...
import type { CommitteesContextValue } from '@/lib/committees/committees-portal-config';
...
import {
  fetchTeacherCommitteesBrowse,
  fetchTeacherCommitteesMine,
} from '@/lib/teacher/teacher-portal-api';

const TeacherCommitteesContext = createContext<CommitteesContextValue | null>(null);

const committeesCache = createTeacherPortalCache<ParentCommitteesData>('teacher_committees:');

function cacheKey(organizationId: string, slug: string): string {
  return `${organizationId}:${slug}`;
}

async function fetchAndCacheTeacherCommittees(
  ...
  return committeesCache.fetchAndCache(
    ...
    async () => {
      const [browseCommittees, myCommittees] = await Promise.all([
        fetchTeacherCommitteesBrowse(organizationId),
        fetchTeacherCommitteesMine(organizationId),
      ]);
...
export function prefetchTeacherCommittees(organizationId: string, slug: string): Promise<void> {
  const key = cacheKey(organizationId, slug);
  return committeesCache.prefetch(key, () => fetchAndCacheTeacherCommittees(organizationId, slug));
}

export async function hydrateTeacherCommitteesFromDisk(
  organizationId: string,
  slug: string,
...
type TeacherCommitteesProviderProps = {
  children: ReactNode;
  organizationId: string;
  slug: string;
};

export function TeacherCommitteesProvider({
  children,
  organizationId,
  slug,
}: TeacherCommitteesProviderProps) {
  const key = cacheKey(organizationId, slug);
  const cached = committeesCache.get(key);

  const [browseCommittees, setBrowseCommittees] = useState(cached?.browseCommittees ?? []);
  ...
  const load = useCallback(
    async (options?: { refresh?: boolean }) => {
      ...
      const run = async () => {
        ...
        try {
          const nextData = await fetchAndCacheTeacherCommittees(organizationId, slug, {
            refresh: isRefresh,
          });
          applyData(nextData);
          setHasLoaded(true);
  ...
  useEffect(() => {
    ...
    async function init() {
      const resolved = await resolveTeacherPortalProviderInit(key, committeesCache, () =>
        hydrateTeacherCommitteesFromDisk(organizationId, slug),
      );
      if (cancelled) return;
      ...
      try {
        await fetchAndCacheTeacherCommittees(organizationId, slug);
        if (cancelled) return;
  ...
  const getBrowseCommittee = useCallback(
    (committeeId: string) =>
      browseCommittees.find((committee) => committee.id === committeeId) ?? null,
    [browseCommittees],
  );
  ...
  return (
    <TeacherCommitteesContext.Provider value={value}>{children}</TeacherCommitteesContext.Provider>
  );
}

export function useTeacherCommittees(): CommitteesContextValue {
  const context = useContext(TeacherCommitteesContext);
  if (!context) {
    throw new Error('useTeacherCommittees must be used within TeacherCommitteesProvider');
  }
  return context;
}
