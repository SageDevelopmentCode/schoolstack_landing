...
# CI APK embeds the JS bundle at build time (see build-android-debug-ci.sh).
# Metro is optional for local debugging only.
if [[ "${ANDROID_E2E_USE_METRO:-}" == "1" ]]; then
  cat > .env.e2e.ci <<EOF
...
EOF
  ...
  echo "Warming Android bundle from Metro..."
