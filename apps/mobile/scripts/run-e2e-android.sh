...
if [[ ! -f "$env_file" ]]; then
  ...
  echo "Create it from the Android block in apps/mobile/.env.e2e.example" >&2
  echo "Or run: npm run mobile:e2e:env -- android > apps/mobile/.env.e2e.local" >&2
  exit 1
fi
...
if ! curl -sf "http://127.0.0.1:3100" >/dev/null 2>&1; then
  ...
  echo "Run: DISABLE_OUTBOUND_EMAIL=1 npm run dev:next -- -p 3100" >&2
fi

if ! curl -sf "http://127.0.0.1:8081/status" >/dev/null 2>&1; then
  echo "Warning: Metro does not appear to be running on :8081" >&2
  echo "Run: cd apps/mobile && set -a && source .env.e2e.local && set +a && npm run android" >&2
fi
