        ...
        <div className="grid grid-cols-1 lg:grid-cols-[5fr_7fr] gap-16 items-center">
          ...
          <div>
            ...
            <FadeInView delay={0.06}>
              <p className="font-display text-[clamp(1rem,1.8vw,1.15rem)] text-text-muted leading-snug mt-4">
                Running a microschool exposed everything regular software misses.
              </p>
            </FadeInView>
            ...
            <FadeInView delay={0.18}>
              <p className="text-[16px] text-text-muted leading-relaxed mt-5">
                ...
                module exists because it solved an actual operational bottleneck
                inside the school.
              </p>
            </FadeInView>
          ...
          <FadeInView delay={0.12}>
            <div className="relative h-[440px] lg:h-[500px]">
              {/* Card 1 — back */}
              <div
                className="absolute top-0 right-8 left-0 h-[300px] rounded-xl bg-surface border border-border shadow-xs overflow-hidden"
                style={{
              ...
              {/* Card 2 — mid */}
              <div
                className="absolute top-8 right-4 left-4 h-[300px] rounded-xl bg-surface border border-border shadow-md overflow-hidden"
                style={{
...
function SnippetPayments() {
  return (
    <div className="p-4 h-full text-[10px]">
      <div className="text-[11px] font-semibold text-text font-body mb-2">
        Transactions
      </div>
      <div className="bg-surface border border-border rounded-lg overflow-hidden">
        {[
          ...
          <div
            key={name}
            className="flex items-center px-3 py-2 border-b border-border last:border-0 gap-2"
          >
            ...
            <span
              className={`text-[9px] rounded-pill px-1.5 py-0.5 ${status === "Paid" ? "bg-green-50 text-green-700" : "bg-yellow-50 text-yellow-700"}`}
            >
              {status}
            </span>
          </div>
        ...
        {[
          ...
          <div key={col.title} className="bg-surface-soft rounded-md p-2">
            <div className="flex items-center justify-between mb-1.5">
              ...
              <span
                className={`text-[9px] rounded-pill px-1 py-0.5 ${col.color}`}
              >
                {col.count}
              </span>
            </div>
            {Array.from({ length: Math.min(col.count, 2) }).map((_, i) => (
              <div
                key={i}
                className="bg-surface border border-border rounded p-1.5 mb-1 text-[9px] text-text-muted"
              >
                <div className="font-medium text-text">Student {i + 1}</div>
                <span className="bg-accent-soft text-accent rounded-pill px-1 py-0.5">
                  Elementary
                </span>
              </div>
            ))}
...
function SnippetParents() {
  return (
    <div className="p-4 h-full text-[10px]">
      <div className="text-[11px] font-semibold text-text font-body mb-2">
        Parent Portal — Enrollment Checklist
      </div>
      <div className="bg-surface border border-border rounded-lg p-3 space-y-2">
        {[
          ...
          <div key={item.label} className="flex items-center gap-2">
            <div
              className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 text-[8px] ${item.done ? "bg-accent text-white" : "border border-border"}`}
            >
              {item.done && "✓"}
            </div>
