...
import TeacherDocCategoryHeading from "@/components/school-teacher/home/TeacherDocCategoryHeading";
...
type TeacherHomeHowToGuidesSectionProps = {
  theme: ParentThemeTokens;
  documentationHref: string;
  groupedGuides: Array<{ category: string; guides: TeacherDocGuide[] }>;
  onOpenGuide: (guide: TeacherDocGuide) => void;
};

export default function TeacherHomeHowToGuidesSection({
  theme,
  documentationHref,
  groupedGuides,
  onOpenGuide,
}: TeacherHomeHowToGuidesSectionProps) {
  return (
      ...
      <div className="mt-4 grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2 xl:grid-cols-3">
        {groupedGuides.map((group) => (
          <section key={group.category}>
            <TeacherDocCategoryHeading category={group.category} theme={theme} />
