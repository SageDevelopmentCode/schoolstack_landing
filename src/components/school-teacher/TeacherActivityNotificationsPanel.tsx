...
import TeacherActivityNotificationRow from "@/components/school-teacher/TeacherActivityNotificationRow";
...
export default function TeacherActivityNotificationsPanel({
  open,
  onClose,
  organizationId,
  slug,
  ...
  onMarkedRead,
}: TeacherActivityNotificationsPanelProps) {
  ...
  const fetchPage = useCallback(
    async (cursor: string | null, append: boolean) => {
      ...
      try {
        ...
        if (previewMode && previewStaffMemberId) {
          const params = new URLSearchParams({
            staffMemberId: previewStaffMemberId,
            limit: String(PAGE_SIZE),
          });
          if (cursor) params.set("cursor", cursor);
          response = await fetch(
            `/api/admin/organizations/${organizationId}/teacher-activity-notifications?${params.toString()}`,
          );
        } else {
          const params = new URLSearchParams({
            organizationId,
    ...
    [organizationId, previewMode, previewStaffMemberId, slug],
  );
    ...
    void (async () => {
      try {
        const response = await fetch(
          "/api/teacher-portal/activity-notifications/mark-read",
          {
            method: "POST",
      ...
      {open ? (
        ...
        >
          ...
          <motion.div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="teacher-activity-notifications-title"
            ...
            >
              <div className="min-w-0">
                ...
                <ParentDisplayHeading
                  ...
                  as="h2"
                  size="section"
                  id="teacher-activity-notifications-title"
                  ...
                  >
                    {notifications.map((notification) => (
                      <motion.li key={notification.id} {...LIST_MOTION}>
                        <TeacherActivityNotificationRow
