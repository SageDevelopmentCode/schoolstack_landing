...
import {
  ...
  type TeacherDocumentationContext,
} from "@/lib/school-teacher/teacher-documentation";
import { searchTeacherDocumentationGuides } from "@/lib/school-teacher/teacher-documentation-search";
...
type TeacherDocumentationPageProps = {
  slug: string;
  schoolName: string;
...
function GuideCard({
  ...
  guide: TeacherDocGuide;
  onOpenGuide: (guide: TeacherDocGuide) => void;
}) {
  return (
    <article
...
export default function TeacherDocumentationPage({
...
}: TeacherDocumentationPageProps) {
  const searchParams = useSearchParams();
  const theme = useMemo(() => buildParentThemeTokens(branding), [branding]);

  const docContext = useMemo<TeacherDocumentationContext>(
    () => ({
      slug,
      features,
      bulletinEnabled,
    ...
    [slug, features, bulletinEnabled, teacherBasePath],
  );

  const allGuides = useMemo(
    () => buildTeacherDocumentationGuides(docContext),
    [docContext],
  );
  ...
  const filteredGuides = useMemo(
    () => searchTeacherDocumentationGuides(allGuides, debouncedQuery),
    [allGuides, debouncedQuery],
  );

  const groupedGuides = useMemo(
    () => groupTeacherDocumentationByCategory(filteredGuides),
    [filteredGuides],
  );
    ...
    <>
      <div className="mx-auto max-w-3xl px-4 py-6 sm:px-6 sm:py-8">
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm" style={{ color: theme.muted }}>
              Step-by-step guides for using the staff portal at {schoolName}.
            </p>
          </div>
        </div>

        <div className="mt-6">
          <label htmlFor="teacher-doc-search" className="sr-only">
            Search guides
          </label>
        ...
        <p className="mt-8 text-xs" style={{ color: theme.muted }}>
          Still stuck? Contact your school administrator for help.
        </p>
      </div>
