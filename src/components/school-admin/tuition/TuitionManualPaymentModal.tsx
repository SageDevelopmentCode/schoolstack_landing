...
export default function TuitionManualPaymentModal({
  ...
  const remainingCents = charge ? chargeRemainingCents(charge) : 0;
  const [amountDraft, setAmountDraft] = useState("");
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (!open || !charge) return;
    setAmountDraft(formatCentsForInput(remainingCents));
    setError(null);
  }, [charge, open, remainingCents]);
  ...
  const handleConfirm = async () => {
    if (!charge || parsedAmountCents == null || validationError) {
