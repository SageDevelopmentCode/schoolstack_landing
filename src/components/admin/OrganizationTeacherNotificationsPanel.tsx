...
type OrganizationTeacherNotificationsPanelProps = {
...
export default function OrganizationTeacherNotificationsPanel({
  organizationId,
  organizationSlug,
  staffMemberId,
  staffLabel,
  open,
  onClose,
}: OrganizationTeacherNotificationsPanelProps) {
    ...
    async (cursor: string | null, append: boolean) => {
      ...
      try {
        ...
        const response = await fetch(
          `/api/admin/organizations/${organizationId}/teacher-activity-notifications?${params}`,
        );
        ...
        const pageNotifications =
          (payload.notifications as TeacherActivityNotification[]) ?? [];
          ...
          >
            <div className="flex flex-shrink-0 items-start justify-between gap-3 border-b border-admin-border px-4 py-3 sm:px-5">
              <div className="min-w-0">
                <h2
                  id="teacher-notifications-title"
                  className="text-sm font-semibold text-admin-text"
                >
                  Teacher notifications
                </h2>
                <p className="mt-0.5 text-xs text-admin-muted">
                  {staffLabel} · {organizationSlug}
                </p>
                <p className="mt-1 text-[11px] text-admin-faint">
                  Read-only — does not mark notifications read for this staff member.
                </p>
              </div>
              <button
                type="button"
