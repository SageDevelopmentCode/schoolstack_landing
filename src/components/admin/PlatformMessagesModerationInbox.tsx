...
type PlatformMessagesModerationInboxProps = {
...
export default function PlatformMessagesModerationInbox({
  organizationId,
  layout = "standalone",
  listHeader,
}: PlatformMessagesModerationInboxProps) {
  ...
  const masterDetailClassName =
    layout === "embedded" ? "h-full" : "h-[calc(100vh-3rem)]";

  const inbox = (
    <AdminMasterDetail
      className={masterDetailClassName}
      list={
        <>
          {listHeader ?? (
            <AdminListPanelHeader>
              <div className="space-y-1">
                <h2 className="text-sm font-semibold text-admin-text">Portal messages</h2>
                <p className="text-xs text-admin-muted">
                  Parent, teacher, and school office conversations.
                </p>
              </div>
            </AdminListPanelHeader>
          )}
