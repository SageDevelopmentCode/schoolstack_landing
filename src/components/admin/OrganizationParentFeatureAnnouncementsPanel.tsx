...
} from "@/lib/parent-portal/parent-feature-announcements";
import { AdminSelect } from "@/components/admin/ui/AdminSelect";

type OrganizationParentFeatureAnnouncementsPanelProps = {
...
function portalScopeLabel(scope: ParentFeatureAnnouncementPortalScope) {
  if (scope === "main") return "Main school portal";
  if (scope === "coop") return "Co-op portal";
  return "Any portal";
}
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      ...
      <label className="block space-y-1">
        ...
        <AdminSelect
          ...
          onChange={(event) =>
            onChange({
              ...form,
              ctaLabel: event.target.value as ParentFeatureAnnouncementCtaLabel,
            })
          }
          className="w-full"
          triggerClassName="px-3 py-2"
        >
          {PARENT_FEATURE_ANNOUNCEMENT_CTA_LABELS.map((label) => (
      ...
      <label className="block space-y-1">
        ...
        <AdminSelect
          ...
          triggerClassName="px-3 py-2"
        >
          {PARENT_FEATURE_ANNOUNCEMENT_FEATURE_KEYS.map((key) => (
      ...
      <label className="block space-y-1">
        ...
        <AdminSelect
          value={form.portalScope}
          onChange={(event) =>
            onChange({
              ...form,
              portalScope: event.target.value as ParentFeatureAnnouncementPortalScope,
            })
          }
          className="w-full"
          triggerClassName="px-3 py-2"
        >
          {PARENT_FEATURE_ANNOUNCEMENT_PORTAL_SCOPES.map((scope) => (
            <option key={scope} value={scope}>
              {portalScopeLabel(scope)}
            </option>
          ))}
        </AdminSelect>
      </label>
      <label className="block space-y-1">
        ...
        <input
          ...
          className="w-full text-sm border border-admin-border rounded-admin-md px-3 py-2 bg-admin-bg font-mono"
        />
        <span className="text-[11px] text-admin-faint font-secondary">
          e.g. curriculum, supply_list, documentation, portal
        </span>
      </label>
      <label className="block space-y-1">
        <span className="text-xs text-admin-muted font-secondary">
...
function recordToForm(
  record: ParentFeatureAnnouncementRecord,
  ...
  return {
    ...
    portalScope: record.portalScope,
    publishedAt: record.publishedAt,
    published: record.published,
  };
}

export default function OrganizationParentFeatureAnnouncementsPanel({
  organizationId,
  organizationName,
}: OrganizationParentFeatureAnnouncementsPanelProps) {
  const [globals, setGlobals] = useState<ParentFeatureAnnouncementRecord[]>([]);
  const [overrides, setOverrides] = useState<ParentFeatureAnnouncementRecord[]>([]);
  const [effective, setEffective] = useState<ParentFeatureAnnouncementRecord[]>([]);
  ...
  const loadAnnouncements = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/organizations/${organizationId}/parent-feature-announcements`,
      );
      const payload = (await response.json()) as {
        globals?: ParentFeatureAnnouncementRecord[];
        overrides?: ParentFeatureAnnouncementRecord[];
        effective?: ParentFeatureAnnouncementRecord[];
        error?: string;
      };

      if (!response.ok) {
        throw new Error(payload.error ?? "Failed to load parent portal cards.");
      }
      ...
      setError(
        loadError instanceof Error
          ? loadError.message
          : "Failed to load parent portal cards.",
      );
      setGlobals([]);
      setOverrides([]);
      setEffective([]);
  ...
  const submitGlobal = async (event: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/admin/parent-feature-announcements", {
        method: "POST",
  ...
  const saveEdit = async () => {
    ...
    try {
      const url = editingGlobalId
        ? `/api/admin/parent-feature-announcements/${editingGlobalId}`
        : `/api/admin/organizations/${organizationId}/parent-feature-announcements/${editingOverrideId}`;
  ...
  const deleteGlobal = async (id: string) => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/admin/parent-feature-announcements/${id}`, {
        method: "DELETE",
      });
      const payload = (await response.json()) as { error?: string };
      if (!response.ok) {
  ...
  const deleteOverride = async (id: string) => {
    ...
    try {
      const response = await fetch(
        `/api/admin/organizations/${organizationId}/parent-feature-announcements/${id}`,
        { method: "DELETE" },
      );
      const payload = (await response.json()) as { error?: string };
      if (!response.ok) {
  ...
  const hideGlobalForOrg = async (global: ParentFeatureAnnouncementRecord) => {
    setSaving(true);
    setError(null);
    try {
      const response = await fetch(
        `/api/admin/organizations/${organizationId}/parent-feature-announcements`,
        {
          ...
          body: JSON.stringify({
            ...
            portalScope: global.portalScope,
  ...
  const renderTable = (
    rows: ParentFeatureAnnouncementRecord[],
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-xs text-admin-muted font-secondary">
                    ...
                    {portalScopeLabel(row.portalScope)} ·{" "}
        ...
        <>
          <section className="bg-admin-surface border border-admin-border rounded-admin-md p-4 space-y-4">
            <div>
              ...
              <p className="mt-1 text-sm text-admin-muted font-secondary">
                Cards shown on parent home pages unless overridden for a school.
              </p>
            </div>
          ...
          <section className="bg-admin-surface border border-admin-border rounded-admin-md p-4 space-y-3">
            <div>
              ...
              <p className="mt-1 text-sm text-admin-muted font-secondary">
                Published cards after portal scope, feature gating, and the 14-day
                window on parent home.
              </p>
            </div>
            {effective.length > 0 ? (
                ...
                {effective.map((row) => (
                  ...
                  >
                    ...
                    <p className="mt-1 text-xs text-admin-muted font-secondary">
                      ...
                      {row.featureKey} · {portalScopeLabel(row.portalScope)}
                    </p>
                  </li>
                ))}
              </ul>
