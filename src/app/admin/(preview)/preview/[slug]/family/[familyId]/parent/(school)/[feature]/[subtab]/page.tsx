...
type PageProps = {
  params: Promise<{ slug: string; familyId: string; feature: string; subtab: string }>;
};
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    title: `${subtabLabel} · ${parentLabel} · ${org.name} Parent Preview`,
  };
}

export default async function FamilyPreviewParentSubtabPage({ params }: PageProps) {
  const { slug, familyId, feature, subtab } = await params;
  const cookieStore = await cookies();
  const supabase = createClient(cookieStore);
  const org = await fetchOrganizationWithSettings(supabase, slug);
