...
export async function createObservationSlot(
  ...
  void logActivityEvent(supabase, {
    ...
    summary: `Opened shadow slot on ${input.date}`,
    metadata: {
      date: input.date,
      open: true,
...
export async function deleteObservationSlot(
  ...
  if (data) {
    void logActivityEvent(supabase, {
      ...
      surface: "school_admin",
      action: ACTIVITY_ACTIONS.AVAILABILITY_SLOT_TOGGLED,
      summary: `Closed shadow slot on ${String(data.date)}`,
      metadata: {
...
export const STUDENT_GRADE_FIELD_ID: ApplySystemFieldId = "student_grade";
