import { compressImageForUpload } from "@/lib/images/compress-image";
import { validateStudentPhotoFile } from "@/lib/students/student-photo-storage";

export class StudentProfilePhotoClientError extends Error {
  code: string;

  constructor(message: string, code: string) {
    super(message);
    this.name = "StudentProfilePhotoClientError";
    this.code = code;
  }
}

export async function uploadStudentProfilePhotoFromParent(
  organizationId: string,
  studentId: string,
  file: File,
): Promise<string> {
  validateStudentPhotoFile(file);

  const prepared = await compressImageForUpload(file, {
    maxWidth: 512,
    maxHeight: 512,
    quality: 0.82,
    skipBelowBytes: 80 * 1024,
  });

  const formData = new FormData();
  formData.append("organizationId", organizationId);
  formData.append("file", prepared);

  const response = await fetch(
    `/api/parent-portal/students/${encodeURIComponent(studentId)}/profile-photo`,
    {
      method: "POST",
      body: formData,
    },
  );

  const payload = (await response.json().catch(() => null)) as
    | { profilePhotoUrl?: string; error?: string; code?: string }
    | null;

  if (!response.ok) {
    throw new StudentProfilePhotoClientError(
      payload?.error ?? "Failed to upload photo.",
      payload?.code ?? "upload_failed",
    );
  }

  const profilePhotoUrl = payload?.profilePhotoUrl?.trim();
  if (!profilePhotoUrl) {
    throw new StudentProfilePhotoClientError(
      "Upload succeeded but no photo URL was returned.",
      "missing_url",
    );
  }

  return profilePhotoUrl;
}
