...
import {
  applyChargeLabelsToTuitionPayments,
...
describe("mapParentTuitionPaymentRows", () => {
  ...
  it("prefers the current charge label over a stale payment label", () => {
    const rows = mapParentTuitionPaymentRows(
      [
        {
          id: "payment-1",
          ...
          currency: "USD",
          status: "succeeded",
          paid_at: "2026-09-06T17:39:05.518+00",
          created_at: "2026-09-06T17:38:07.422955+00",
        },
      ],
      "family-1",
      new Map(),
      new Map([["charge-1", "Sep Tuition (Zachary)"]]),
    );

    assert.equal(rows[0]?.label, "Sep Tuition (Zachary)");
  });
...
describe("applyChargeLabelsToTuitionPayments", () => {
  it("replaces stale payment labels when a charge label is available", () => {
    const payments = applyChargeLabelsToTuitionPayments(
      [
        paymentRecord({
          label: "Aug Tuition (Zachary)",
          tuitionChargeId: "charge-zachary",
        }),
      ],
      new Map([["charge-zachary", "Sep Tuition (Zachary)"]]),
    );

    assert.equal(payments[0]?.label, "Sep Tuition (Zachary)");
  });
});
